...
describe("NFT Auction", function () {
  ...
  beforeEach(async () => {
    [owner, projectTreasury, author, other, bidder1, bidder2, ...addrs] =
  ...
  describe("Set NFT", () => {
    ...
    it("should reverts for non nft contract", async function () {
      await expect(auctionContract.setNFTContract(other.address)).to.be
        .reverted;
    });
  });

  it("Pause and Unpause", async function () {
    await expect(await auctionContract.connect(owner).pause())
      .to.emit(auctionContract, "Paused")
      .withArgs(owner.address);
    await expect(
      auctionContract.connect(owner).createAuction(0, MIN_BID, MAX_BID, 360)
    ).to.be.revertedWith("Pausable: paused");
    await expect(await auctionContract.connect(owner).unpause())
  ...
  describe("Create Auction", () => {
    ...
    it("allows the author to create an auction", async function () {
      expect(
        await auctionContract
          .connect(author)
          .createAuction(0, MIN_BID, MAX_BID, 360)
      )
        .to.emit(auctionContract, "AuctionCreated")
        .withArgs(0, MIN_BID, MAX_BID, 360);
    ...
    it("allows multiple auctions", async function () {
      ...
      await expect(
        await auctionContract
          .connect(owner)
          .createAuction(1, MIN_BID, MAX_BID.mul(2), 3600)
      )
        .to.emit(auctionContract, "AuctionCreated")
        .withArgs(1, MIN_BID, MAX_BID.mul(2), 3600);
    ...
    it("should revert when a duplicate auction is created", async function () {
      const mintTx = await auctionContract
        .connect(owner)
        .createAuction(0, MIN_BID, MAX_BID, 360);
      await mintTx.wait();
      const auciontData = await auctionContract.getAuction(0);
    ...
    it("should revert on nonexistent auctions", async function () {
      await expect(auctionContract.getAuction(3)).to.be.revertedWith(
        "Not Auction"
      );
    });
  });
  describe("Bid", () => {
    beforeEach(async () => {
      expect(await auctionContract.setNFTContract(nft.address));
      const mintTx2 = await auctionContract.createAuction(
        0,
        MIN_BID,
        MAX_BID,
        360
    ...
    it("allows users to bid eth successfully", async function () {
      await expect(
        ...
        .to.emit(auctionContract, "AuctionBid")
        .withArgs(0, LOW_BID, bidder1.address);
    ...
    it("should revert for bids below last bid", async function () {
      await expect(
        ...
        .to.emit(auctionContract, "AuctionBid")
        .withArgs(0, HIGH_BID, bidder1.address);
      await expect(
        auctionContract.connect(bidder2).bid(0, { value: LOW_BID })
      ).to.be.revertedWith("bid below last bid");
    });
    it("should return change for bids over max price", async function () {
      ...
      await expect(
        ...
        .to.emit(auctionContract, "AuctionBid")
        .withArgs(0, MAX_BID, bidder1.address);
      let balance2_bidder1 = await bidder1.getBalance();
      expect(
        balance1_bidder1
          .sub(balance2_bidder1)
          .sub(MAX_BID)
          .lt(parseEther("0.001"))
      ).to.be.true;
      await expect(
    ...
    it("multiple bids with payback", async function () {
      ...
      await expect(
        ...
        .to.emit(auctionContract, "AuctionBid")
        .withArgs(0, HIGH_BID, bidder2.address);
    ...
    it("is resilient to malicious bids", async function () {
      const MaliciousBidderMock = await ethers.getContractFactory(
        "MaliciousBidderMock",
        owner
      );
      ...
      await expect(
        await maliciousBidder
          .connect(other)
          .bid(auctionContract.address, 0, { value: MAL_BID })
      )
        .to.emit(auctionContract, "AuctionBid")
        .withArgs(0, MAL_BID, maliciousBidder.address);
      auciontData = await auctionContract.getAuction(0);
      ...
      await expect(
        ...
        .to.emit(auctionContract, "FailedPayment")
        .withArgs(maliciousBidder.address, MAL_BID)
        .and.to.emit(auctionContract, "AuctionBid")
        .withArgs(0, HIGH_BID, bidder2.address);
      auciontData = await auctionContract.getAuction(0);
      expect(auciontData[5]).to.equal(HIGH_BID);
      expect(auciontData[6]).to.equal(bidder2.address);
      ...
      expect(
        await ethers.provider.getBalance(maliciousBidder.address)
      ).to.equal(0);
      expect(
        await ethers.provider.getBalance(auctionContract.address)
      ).to.equal(MAL_BID.add(HIGH_BID));
  ...
  describe("Cancel Auction", () => {
    ...
    it("allows author to cancel auction", async function () {
      await expect(await auctionContract.connect(author).cancelAuction(0))
        .to.emit(auctionContract, "AuctionCancelled")
        .withArgs(0);
      await expect(auctionContract.getAuction(0)).to.be.revertedWith(
        "Not Auction"
      );
    });
  ...
  describe("Admin Cancel Auction", () => {
    ...
    it("should revert if anyone else try to cancel auction", async function () {
      await expect(auctionContract.connect(other).pause()).to.be.revertedWith(
  ...
  describe("Finish Auction", () => {
    ...
    it("ends auction by time", async function () {
      await expect(
        ...
        .withArgs(0, LOW_BID, bidder1.address);
      await ethers.provider.send("evm_increaseTime", [1000]);
      await expect(await auctionContract.connect(other).finishAuction(0))
        .to.emit(auctionContract, "AuctionFinish")
        .withArgs(0, LOW_BID, bidder1.address);
  ...
  describe("Withdraw Unclaimed", () => {
    ...
    it("allows owner to withdraw unclaimed eth", async function () {
      ...
      await expect(await auctionContract.connect(owner).pause())
        ...
        .withArgs(owner.address);
      await expect(
        await auctionContract.connect(owner).withdrawUnclaimed(owner.address)
      );
    });
